...
class LSTM_Network(keras.Model):
    ...
    def _build_net(self):
        ...
        self.model.add(Bidirectional(
            LSTM(units=48, input_shape=self._input_shape, return_sequences=False, recurrent_activation="softsign",
                 activation="tanh", recurrent_regularizer="l2")))
        # Dropout layer 1
        self.model.add(Dropout(rate=self._dropout_rate))
        # # LSTM layer 2
        # self.model.add(Bidirectional(
        #     LSTM(units=32, input_shape=[self._input_shape[0], 48], return_sequences=False,
        #          recurrent_activation="softsign",
        #          activation="tanh", recurrent_regularizer="l2")))
        # # Dropout layer 2
        # self.model.add(Dropout(rate=self._dropout_rate))
        # Dense layer
        self.model.add(Dense(units=24, input_shape=[self._input_shape[0], 32], activation="relu"))
        # Dropout layer 3
        self.model.add(Dropout(rate=self._dropout_rate))
        # Output layer
        self.model.add(Dense(units=self._output_shape[1], input_shape=[self._input_shape[0], 24], activation="softmax"))
